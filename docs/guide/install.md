...
├── system.phrases.toml               # 覆盖内置系统短语种子（可选，一般不需要；旧版 .yaml 仍可读）
├── schemas\                          # 输入方案与方案级词库
│   ├── pinyin.schema.toml            # 方案配置（覆盖或新增；旧版 .schema.yaml 仍可读）
│   ├── wubi86.schema.toml
│   ├── my_schema.schema.toml         # 自定义新方案
│   ├── <方案词库>.dict.yaml           # 方案引用的 RIME 词库（仍为 YAML 格式）
...
| 自定义短语              | 通过 **设置 → 词库 → 短语** 添加（写入 `user_data.db`），不要直接编辑 `system.phrases.toml` |
