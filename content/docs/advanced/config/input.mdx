...
| `symbol_chars` | 字符串 | — | `"0123456789+-_.@#/"` | 允许进入缓冲的字符白名单<Since v="0.115" />，**逐字符字面匹配**，不支持 `a-z` 这类范围写法。未列入的字符保持原有职责（详见[临时英文的字符白名单](/docs/input/temp-modes#temp-english-symbol-chars)）。留空则一个都不放行 |
