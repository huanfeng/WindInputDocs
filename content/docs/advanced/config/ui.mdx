...
注意区分职责：**纯视觉样式**（颜色、圆角、透明度、字号基线、边距等）由**主题**决定，见[外观与主题](/docs/customize/appearance)；本页配置的是**行为与布局**（显示什么、怎么排、放哪里）。二者交汇处（如候选字号、翻页栏显隐）本页字段可覆盖主题基线。
...
| `comment_template_vertical` | 字符串 | — | `"${code_hint\|code}"` | **竖排**候选注释（右侧灰字）模板。语法与变量见[候选注释](/docs/customize/candidate-comment)；留空 = 不显示 |
...
主题的具体样式定义（配色、圆角、字号基线等）见[外观与主题](/docs/customize/appearance)。
...
- [外观与主题](/docs/customize/appearance) — 主题包与视觉样式（配色 / 圆角 / 字号基线）
