...
| 键 | 类型 | 说明 |
| --- | --- | --- |
| `special_modes` | 结构体数组 | 特殊模式列表（各自带码表 + 上屏策略，引导键触发）。子字段见[临时模式](/docs/advanced/config/special-modes) |
| `mix_modes` | 结构体数组 | 临时混输模式列表（引导键触发，合并多个成员方案的候选）。子字段见[临时模式](/docs/advanced/config/special-modes) |
