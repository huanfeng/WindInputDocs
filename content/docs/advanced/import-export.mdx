...
在**设置 → 词库 → 快捷短语**中导入导出，格式同为 WindDict，只含 `phrases` 一段（默认文件名 `phrases.wdict.yaml`）。短语独有 `position` 列（同编码同权重时的先后），导入会保留该列的值。详见[自定义短语](/docs/customize/phrases)。
...
**设置 → 高级 → 备份与还原**。操作流程见[备份与还原](/docs/advanced/backup)，这里只讲格式。
...
- [备份与还原](/docs/advanced/backup) —— 一键备份的操作流程与换机迁移
- [词库与词频](/docs/customize/dictionaries) —— 词库管理界面与调频原理
- [自定义短语](/docs/customize/phrases) —— 短语字段与 `position`
- [自定义方案](/docs/customize/schemas) —— 方案文件结构与引用路径
- [配置文件与数据目录](/docs/advanced/config) —— 各目录职责与覆盖机制
