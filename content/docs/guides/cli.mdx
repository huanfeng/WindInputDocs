...
| `restart` | 均可 |
| `ui` <Since v="0.121" /> | **需要** |
| `system` <Since v="0.121" /> | 不需要（但要管理员权限） |
| `help` / `--version` | 不需要 |
...
| `config check [--custom <目录>] [--data <目录>]` <Since v="0.120" /> | 体检[定制版数据层](/docs/guides/custom-edition)，打包前自查 |
...
- 主题包的格式与大小限制见[主题包](/docs/settings/appearance/themes#wtheme)
...
可还原的域：`config`、`dict`、`temp`、`freq`、`shadow`、`phrase`、`common_chars`、`charset`、`completion` <Since v="0.122" />、`schemas`、`themes`、`state`、`stats`。
...
提示只能由输入法主进程弹出，故本命令**需要服务在线**。短语里的等价写法是 [`ui.toast(...)`](/docs/guides/command-bar#ui-toast)，两者共用同一套校验。

## system <Since v="0.121" /> [#system]
...
Dota 2 按输入法在系统里登记的**名称**查一张内置白名单，决定要不要由游戏自己绘制候选；不在表内就取不到候选，还会多出一个左上角的系统 IME 小窗。开启后本输入法登记的名称改为 `--name` 给的那个（省略则用默认的「拼音输入法」）——**语言栏、Windows 设置、输入法切换列表里显示的都会是那个名字**，改完需重启游戏才生效。

名单里多数名称带空格，在命令行里必须**整体加引号**；比对是逐字全等的，差一个字符就不命中且不会报错。可选名称与各自的副作用见[高级设置 · 游戏兼容](/docs/settings/advanced#game-compat)。

日常从设置页操作即可（[高级 → 游戏兼容](/docs/settings/advanced#game-compat)），设置程序会以 `runas` 拉起本子命令，你当场看到一次 UAC。本子命令**不读配置**、目标状态由参数显式给出——提权到另一个管理员账户时读配置会读到那个账户的数据。
...
同样这三个变量在[命令栏短语](/docs/guides/command-bar#内部目录变量)里也能用，写法一致。但两处对路径的**转义规则不同**，照搬时要留意：
...
这组参数同样可以从[命令直通车](/docs/guides/command-bar#setting-open-args)里用 `setting.open("dict", "--schema=wubi86 --type=shadow")` 触发，无需自己拼命令行。
...
照搬终端里跑通的命令时有两点要改：**每个参数写成独立的字符串**（单参形式会按空白拆散，参数自己含空格就散了），以及**字符串里的 `{` `}` 要写成 `\{` `\}`**（大括号在短语里是内插语法）。路径里的反斜杠同样写成 `\\`。两者写错都不报错——前者是配置没变、后者是候选里干脆没有这条短语。详见[照搬终端命令的两道坎](/docs/guides/command-bar#cli-args)。

注意它是发射后不管的，**命令失败不会有任何提示**。

## 相关阅读

- [命令直通车](/docs/guides/command-bar) —— 在短语中执行动作与调用 CLI
- [配置文件与数据目录](/docs/guides/config) —— `config` 子命令的键路径
- [备份与还原](/docs/settings/advanced#备份与还原) —— 图形界面的备份操作
- [输入方案管理](/docs/guides/schemas) —— 方案文件结构与排序配置
