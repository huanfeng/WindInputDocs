...
wind_input system dota2-compat on --name "中文 (简体) - 五笔"   :: 指定登记名称
wind_input system dota2-compat off
wind_input system dota2-compat status   :: 显示当前登记的名称
```

Dota 2 按输入法在系统里登记的**名称**查一张内置白名单，决定要不要由游戏自己绘制候选；不在表内就取不到候选，还会多出一个左上角的系统 IME 小窗。开启后本输入法登记的名称改为 `--name` 给的那个（省略则用默认的「中文 (简体) - 郑码」）——**语言栏、Windows 设置、输入法切换列表里显示的都会是那个名字**，改完需重启游戏才生效。

名称带空格，在命令行里必须**整体加引号**；名单比对是逐字全等的，差一个空格就不命中且不会报错。可选名称与各自的副作用见[高级设置 · 游戏兼容](/docs/settings/advanced#game-compat)。
