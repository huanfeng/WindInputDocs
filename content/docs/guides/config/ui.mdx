...
`min_window_width_*` 与 `max_chars`（候选文本最大字数）成对：一个封窗口下限、一个封文字上限，两个都配上候选窗宽度就基本固定。`min_rows` 与 `min_window_height_vertical` 是两种量法，可同时配、取两者较大者：前者只稳住候选区（翻页栏照常伸缩），后者罩住整个窗口。
