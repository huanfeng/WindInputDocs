...
| `items` | 字符串数组 <Since v="0.119" /> | `mode` / `punct` / `full_width` / `s2t` / `settings` | 全部 | 显示哪几格、按什么顺序——**数组顺序即渲染顺序**。留空 = 全部显示。`s2t` 格还需简繁转换当前开着才出现（本项只决定"允许它出现"）；隐藏 `settings` 不影响使用，右键工具栏任意位置同样能打开菜单。整条工具栏都不想要请用 `visible = false`，不要配成空数组 |
