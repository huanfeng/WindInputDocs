...
候选窗默认完全跟随内容缩放：单字候选与多字候选间来回切换时宽度跳动，竖排下候选数变化又让高度跟着伸缩，连续输入时观感抖动明显。两个下限项用来锁住尺寸，出厂均为 `0`（关闭）。

```toml
min_width_chars = 0   # 候选行内容最小宽度（全角字符数），0 = 关闭
min_rows = 0           # 竖排候选窗最小行数，0 = 关闭
```

| 键 | 类型 | 可选值 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `min_width_chars` | 整数 | — | `0` | 候选行内容的最小宽度，按**全角字符数**计；`0` = 关闭，宽度跟随内容。衡量的是**整行内容**（序号 + 候选文字 + 注释，不含行内边距），不是只有候选文字段。序号列本身约占 2 个汉字宽，实用值通常 `8`~`15`；过小（如 `3`）几乎不起作用。超出下限后宽度照常跟随内容，**不封顶** |
...
`min_width_chars` 与 `max_chars`（候选文本最大字数）成对：一个封下限、一个封上限，两个都配上候选窗宽度就基本固定。`min_rows` 配成与 `per_page` 相同的值，是竖排下把候选窗高度完全固定的最典型用法。
