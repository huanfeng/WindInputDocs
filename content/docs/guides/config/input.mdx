...
| 键 | 类型 | 可选值 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `kind` | 枚举 | `auto` / `off` / `word` / `smart` | `"auto"` | 联想类型，**兼任总开关**（`off` 即关）。`auto` = 按平台定：桌面端关、移动端智能联想 |
| `mode` | 枚举 | `auto` / `one_shot` / `continuous` | `"auto"` | `one_shot` = 只出一次，任何非选词动作即退出；`continuous` = 选中后拿它当新上文接着给（「中」→「中国」→ 以「中国」开头的词）。`auto` = 桌面端一次性、移动端持续 |
