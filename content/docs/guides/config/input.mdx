...
punct_on_empty_behavior = "commit"  # 有编码但无候选时标点键的行为
...
| `punct_on_empty_behavior` <Since v="0.118" /> | 枚举 | `commit` / `clear` | `"commit"` | 有编码但当前无候选时按标点/符号键：`commit` = 上屏「已转换前缀 + 剩余原码」再接标点；`clear` = 丢弃编码，只上屏标点。**与 `schema.codetable.punct_commit` 是两回事**——后者关掉是吞键、连标点都不输出 |
