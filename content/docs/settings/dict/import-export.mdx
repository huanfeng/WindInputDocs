...
| 方案包（方案 + 引用资源） | `.zip`（零层级） | `.zip` / `.wpkg` <Since v="0.118" /> | 设置 → 方案，或高级 → 导入 |
| 配置片段（配置项子集） | —— | `.toml` 文件 / 剪贴板文本 <Since v="0.118" /> | 设置 → 高级 → 导入 |
| 方案文本（小方案纯文本） | —— | 剪贴板文本 <Since v="0.118" /> | 设置 → 高级 → 导入 |
...
[命令栏语法](/docs/guides/command-bar)的条目（`$CC` / `$SS` / `$AA` 与含 `{}` 的模板）是例外：**反斜杠不参与本层转义**，原样进出。那条源码里的 `\` 由表达式解析独占，这里再转一次就是双重展开，路径会静默坏掉。换行与制表仍要转义——TSV 一条记录一行、Tab 分列，不转就把文件切散了。详见[换个地方写，还是两个](/docs/guides/command-bar#escape-layers)。
...
#### 导入时补齐边界 <Since v="0.118" /> [#fill-boundary]
...
单个方案连同其引用资源打成的分发包。在**设置 → 方案**页选中方案后用「导出」/「导入」，也可从[高级页统一导入](/docs/settings/advanced#import-export)进入。

- 新后缀 `.wpkg` <Since v="0.118" />：格式与 `.zip` 完全相同，注册[文件关联](/docs/settings/advanced#desktop-integration)后可双击导入
- 包内可带[随包配置](#config-patch) <Since v="0.118" />：把「装上方案」和「配好相关配置」合成一步
- 导出会把你在设置页对该方案做的定制一并折叠进包 <Since v="0.118" />：定制指向的词库、双拼布局、字根字体也随包走，对方装上即得到与你一样的方案
...
- `format_version` 是包格式版本号 <Since v="0.118" />：缺省视为 1（旧格式，照常导入）；高于当前程序支持的版本则拒绝导入并提示升级。携带[随包配置](#config-patch)的包**必须**声明 `format_version = 2`
...
- `format_version` 高于当前程序支持的版本 → 拒绝，提示升级 <Since v="0.118" />
- 含 `config_patch.toml` 但没有声明 `format_version = 2` → 拒绝（旧版程序会把这份文件当死文件落进方案目录，打包方必须声明版本）<Since v="0.118" />
- 条目超过 2000 个，或解压后总大小超过 256 MB → 拒绝 <Since v="0.118" />（真实方案包远低于此，触发说明包有问题）
...
[输入补全的学习数据](/docs/settings/dict#completion)（邮箱后缀与网址历史）自 0.122 起<Since v="0.122" />一并进包，两类装在同一个 `completions.jsonl` 里，每行自带类别。
...
- 方案文本另有全文 2 MB、64 个文件的上限 <Since v="0.118" />
- `windinput://` 一键安装的方案包下载仅接受 windinput.com 官方域，上限 64 MB，下载后仍走完整的导入确认 <Since v="0.118" />
...
- [配置分发与导入](/docs/guides/import-export) —— 片段 / 方案包 / 方案文本的用法与分享方式 <Since v="0.118" />
