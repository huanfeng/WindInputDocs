...
- `POST /api/comments/admin` —— 放行 / 删除 / 恢复 / 封禁 / 解封 / 切换审核策略 / 开关留言。
...
| 封禁名单 | 按 `ip_hash` 拉黑，并连坐下架该来源全部评论。管理页可解封 |
...
交互上可以一直「回复回复」，**存储永远只有两层**——B 站 / 微博的做法。

回复一条楼中回复时，新评论的 `parent_id` 被提升到同一楼，被回复者记进 `reply_to`，展示为「回复 @某某」：

```
┌─ 张三                          id=19  parent=NULL  reply_to=NULL
│  这个功能怎么开启？
│  ├─ 李四                       id=23  parent=19    reply_to=NULL   ← 回复楼主，不 @
│  │  在设置里第三项
│  ├─ 王五  回复 @李四            id=24  parent=19    reply_to=23
│  │  我这边没有那一项
│  └─ 赵六  回复 @王五            id=25  parent=19    reply_to=24
│     我也没有
```

**层级规则只在 Worker 里实现一处**（`handlePost` 第 5 步）：`parent_id` 要么取目标自身的 `id`（目标是顶层），要么取目标的 `parent_id`（目标在楼中，其父必是顶层）。归纳下来深度恒为 2，伪造请求也造不出第三层。前端只负责报「在回复哪一条」，不参与判断。

这条不变量买到的东西：前端不需要递归组件、不存在孤儿节点与循环引用、一次拉全页评论在客户端 `filter` 两遍就能渲染完——真·树形模型要额外解决的深度上限、删除中间节点的占位、深层折叠，一个都不用碰。手机上第 4 层往后正文也没有可读宽度了，绕一圈还是会退化成 @ 引用。

回复顶层时**不记 @**：楼主的名字就在楼顶，再 @ 一遍是噪音。

被 @ 的那条后来被删或被封禁下架时，前端显示「回复 已删除的评论」而不是省掉整个标记——否则读者会看到一句凭空冒出、不知在回应谁的话。新回复则无法指向已删除的评论（`status` 校验挡在前面）。

`reply_to` 只回 id，不回昵称：整页评论前端都有，查名字是本地一次 Map 命中；服务端 JOIN 是为同一份数据多跑一趟 SQL，还会把「被 @ 的人后来改了昵称」这种不一致烘焙进响应。

**没有回复通知。** 没有账号、不收邮箱，`@` 只能是视觉线索，被回复的人下次来才看得到——这是不收邮箱换来的代价。

### 已部署的库如何升级

```bash
pnpm exec wrangler d1 execute windinput-comments --remote   --command="ALTER TABLE comments ADD COLUMN reply_to INTEGER"
```

`ADD COLUMN` 会给存量行填 `NULL`，正是想要的：老评论都没有 @ 目标。不执行这一步，发表接口会因为列不存在而全部失败。
...
更常用的是直接打开管理页——放行、删除、封禁、解封、切策略、开关留言都在上面，手机也能操作。

管理页自上而下：**总开关**与两组**审核策略**（文档页 / 留言板），**按文档**概览（页面直链 + 公开数 + 待审数），**待审**，**最近公开**，最后是两块默认折叠的 **已删除** 与 **封禁名单**。

删除是可逆的——只改 `status`，正文一直在库里。展开「已删除」即可找到误删的条目，点「恢复」放回去。

### 解封

封禁曾是单向的：`bans` 表只进不出，误封只能翻出装了 wrangler 的电脑手工删行。现在「封禁名单」里每条都有**解除封禁**按钮。

**解封不自动恢复被连坐下架的评论。** `comments` 表没有记录 `status` 的变更时间，无从区分「封禁前就手动删掉的垃圾」与「因封禁连坐下架的」，一并恢复会把真垃圾也放回站上。误封的那几条走「已删除」区逐条恢复——每张卡片的元信息行都带 8 位**来源短码**，与封禁名单里显示的是同一串，肉眼即可对上。

解封不改变任何评论的可见性，因此不清缓存。
